...
Bitmap : Collection {
	...
	*/
	...
	asTask { arg instr = \transping, tempo = 180, threshold = 0.3, 
		scaleAmplitude = true, mul = 0.1, postColumn = true, dur = this.width, type = \matrix, gui = false;
		/*	if(gui, {
			/////// GUI
			var win,bStart,bReset,bFree;
			win = Window.new("GimpOSC",Rect(1280-600, 800-60, 540, 60)).front;
			q.slider = Slider(win, Rect(10,30, 520, 40));
			q.slider.action = { q.slider.value.postln; };
